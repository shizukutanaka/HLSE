...
Binary size:   ~370 KB (dynamic, stripped; arm64), ~1.0 MB (static-pie, stripped; Linux)
Dependencies:  libc + libm only
Platform:      Linux, macOS (no `make static` — Darwin has no static libc)
```
...
| Protection | 24 | Ransomware (incl. R6 intermittent-encryption), network drive, SMB, MBR/GPT, ESP |
