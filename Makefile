...
$(FUZZ_MANIFEST): tests/hlse_manifest_fuzz.c hlse_manifest.c hlse_manifest.h hlse_util.c
	@mkdir -p tests
	$(CC) -O0 -g -Wall -Wextra -D_POSIX_C_SOURCE=200809L $(PLATFORM_CFLAGS) \
		-o $@ tests/hlse_manifest_fuzz.c hlse_manifest.c hlse_util.c -I. -lm
	@printf '  %-20s %s\n' "CC" "$@"

$(FUZZ_MANIFEST_ASAN): tests/hlse_manifest_fuzz.c hlse_manifest.c hlse_manifest.h hlse_util.c
	@mkdir -p tests
	$(CC) -O1 -g -Wall -Wextra -D_POSIX_C_SOURCE=200809L $(PLATFORM_CFLAGS) \
		-fsanitize=address,undefined \
		-o $@ tests/hlse_manifest_fuzz.c hlse_manifest.c hlse_util.c -I. -lm
